...
struct HostRasterizer::Impl {
    ...
    static uint8_t findFree(const std::array<bool, 256> &used) {
        for (int i = 0; i < 256; ++i) if (!used[i]) return static_cast<uint8_t>(i);
        return 0xFF;
    }
    ...
    static void toS3LTransform(const Rasterizer::Transform &t, S3L_Transform3D &out) {
        out.translation.x = toS3L(t.posX);
        out.translation.y = toS3L(t.posY);
        out.translation.z = toS3L(t.posZ);
        out.rotation.x = angleToS3L(t.rotX);
        out.rotation.y = angleToS3L(t.rotY);
        out.rotation.z = angleToS3L(t.rotZ);
        out.scale.x = toS3L(t.scaleX);
        out.scale.y = toS3L(t.scaleY);
        out.scale.z = toS3L(t.scaleZ);
        out.scale.w = 0;
    }

    // Pixel callback
    void drawPixel(S3L_PixelInfo *p) {
        if (p->x < 0 || p->y < 0 || p->x >= fbW || p->y >= fbH) return;
        size_t mIdx = p->modelIndex;
        if (mIdx >= modelToVId.size()) return;
        ...
        if (i0 >= verts.size() || i1 >= verts.size() || i2 >= verts.size()) return;
...
// Draw pixel
static inline void HostRasterizer_DrawPixelShim(S3L_PixelInfo *p) {
    if (HostRasterizer::Impl::g_current) HostRasterizer::Impl::g_current->drawPixel(p);
}
...
HostRasterizer::HostRasterizer(int width, int height)
    ...
    if (!impl->window) throw std::runtime_error(SDL_GetError());
    ...
    if (!impl->renderer) throw std::runtime_error(SDL_GetError());

    impl->texture = SDL_CreateTexture(impl->renderer, SDL_PIXELFORMAT_ABGR8888,
                                      SDL_TEXTUREACCESS_STREAMING, width, height);
    if (!impl->texture) throw std::runtime_error(SDL_GetError());
...
void HostRasterizer::rect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint32_t argb) {
    ...
    for (uint32_t yy = Y0; yy < Y1; ++yy) {
        ...
        for (uint32_t xx = X0; xx < X1; ++xx) row[xx] = abgr;
    }
}
...
Rasterizer::WipeAllResponse HostRasterizer::wipeAll() {
    for (auto &b: impl->vused) b = false;
    for (auto &b: impl->tused) b = false;
    for (auto &v: impl->vbuf) v.clear();
    for (auto &v: impl->tbuf) v.clear();
    for (auto &v: impl->s3lVerts) v.clear();
    for (auto &v: impl->s3lTris) v.clear();
...
Rasterizer::CreateVertResponse HostRasterizer::createVertex(const Rasterizer::Vertex *vertices, uint16_t count) {
    ...
    dst.resize(count);
    for (uint16_t i = 0; i < count; ++i) dst[i] = vertices[i];
...
Rasterizer::CreateTriResponse HostRasterizer::createTriangle(const Rasterizer::Triangle *triangles, uint16_t count) {
    ...
    dst.resize(count);
    for (uint16_t i = 0; i < count; ++i) dst[i] = triangles[i];
